...
const Login = () => {
  ...
  // Gestion du token Google OAuth depuis le callback redirect
  useEffect(() => {
    ...
    if (errorFromUrl) {
      return; // L'erreur sera ignorée ici, le composant s'affiche normalement
    }

    if (tokenFromUrl) {
      // Stocker le token avec le nom normalisé sora_token
      localStorage.setItem('sora_token', tokenFromUrl);
      
      // Récupérer les infos user via une requête API
      const doLogin = async () => {
        try {
          const res = await axios.get(`${AUTH_API_URL}/me`, {
            headers: { Authorization: `Bearer ${tokenFromUrl}` }
          });
          login({ user: res.data.user, token: tokenFromUrl });
        } catch {
          // Fallback: on connecte avec juste l'email
          login({ user: { email: emailFromUrl, name: emailFromUrl?.split('@')[0] || 'Utilisateur' }, token: tokenFromUrl });
        }
        navigate('/');
      };
      doLogin();
